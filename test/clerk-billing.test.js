...
const { CLERK_API_VERSION, getUserBillingSubscription, normalizeSubscription } = require('../clerk-billing');
...
test('normalizeSubscription selects the active paid item and deduplicates features', () => {
  ...
  }), { plan: 'growth', features: ['audit', 'webhooks'], status: 'active' });
});
...
test('getUserBillingSubscription uses the versioned Clerk Backend API without exposing the secret', async () => {
  ...
  assert.deepEqual(result, { plan: 'starter', features: ['audit'], status: null });
